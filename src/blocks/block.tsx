...
export function Block(props: BlockData) {
  ...
  const { color, level = 0 } =
    useSelector((state: RootState) => state.block.blocks[id]) || {};
  const childMap =
    useSelector((state: RootState) => state.block.isChildren[id]) || {};
  const childOrder = useSelector(
    (state: RootState) => state.block.childrenOrder[id] || []
  );
  ...
  return (
    <>
      ...
      >
        ...
        <DragDeletable
          style={{ marginLeft }}
          onDelete={() => dispatch(deleteBlock(id))}
        >
          <Box
            flexDirection={"row"}
            display={"flex"}
